...
class Student {
    ...
    feeForCourse = 0;
    static counter = 1;
    constructor(id, name, course, feespaid, feeForCourse) {
        this.id = id;
        this.name = name;
        this.course = course;
...
class StudentManagementSystem {
    ...
    async displayStudentStatus() {
        ...
        let student = this.students.find((v) => v.id === studentStatus.id);
        if (student) {
    ...
    enrollStudent(name, course, feesPaid) {
        const id = Student.generatePaddedId();
        const newStudent = new Student(id, name, course, feesPaid, this.courseFee);
    ...
    async tutionFee() {
        ...
        let student = this.students.find((s) => s.id == parseInt(answer.id));
        if (student) {
